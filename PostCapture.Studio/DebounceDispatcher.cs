...
namespace PostCapture.Studio
{
    ...
    public class DebounceDispatcher
    {
        ...
        /// </summary>
        /// <param name="intervalMilliseconds">Timeout in Milliseconds</param>
        /// <param name="action">Action<object> to fire when debounced event fires</object></param>
        /// <param name="param">optional parameter</param>
        /// <param name="priority">optional priorty for the dispatcher</param>
        /// <param name="disp">optional dispatcher. If not passed or null CurrentDispatcher is used.</param>
        public void Throttle(int intervalMilliseconds, Action<object> action,
            object param = null,
            DispatcherPriority priority = DispatcherPriority.ApplicationIdle,
            Dispatcher disp = null)
        {
            ...
            if (curTime.Subtract(timerStarted).TotalMilliseconds < intervalMilliseconds)
                intervalMilliseconds -= (int)curTime.Subtract(timerStarted).TotalMilliseconds;

            timer = new DispatcherTimer(TimeSpan.FromMilliseconds(intervalMilliseconds), priority, (s, e) =>
            {
                if (timer == null)
                    return;
